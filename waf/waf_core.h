...
typedef std::pair<distance_type, size_type>
    cooccur_type;  // (mean-distance, count) pair
...
// waf kernel function
// =============================================================================
// pre-condition: *(freq_iter + *max_element(termid_first, termid_last)) is
// accessable
...
void term_frequency(InputIterator termid_first, InputIterator termid_last,
  ...
  for (; termid_first != termid_last; ++termid_first) {
    auto termid = static_cast<termid_type>(*termid_first);
...
inline void comat_enlarge(SparseMatrix<waf::cooccur_type>& co_mat,
...
// pre-condition: *max_element(termid_first, termid_last) <
// co_mat.column_count()
template <typename InputIterator, typename Predicate1, typename Predicate2>
void co_occurrence(InputIterator termid_first, InputIterator termid_last,
  ...
  for (; termid_first != termid_last && term_win.size() < co_win;
       ++termid_first) {
  ...
  while (term_win.size() >= 2) {  // at least contains two terms
    ...
    if (delim_termid != term_win[0] && care_left(term_win[0])) {
      ...
      for (waf::size_type i = 1; i < term_win.size(); ++i) {
        if (delim_termid == term_win[i]) {
          break;
        }
        if (!care_right(term_win[i])) {
          continue;
        }
        ...
        if (term_unique.count(term_win[i])) {
          continue;
        }
...
inline force_type word_activation_force(const cooccur_type& co_info,
                                        size_type left_freq,
                                        size_type right_freq) {
...
affinity_type affinity_or_mean(InputIterator first1, InputIterator last1,
                               Predicate1 care1, InputIterator first2,
                               InputIterator last2, Predicate2 care2,
                               UnaryFunction iter_termid,
                               affinity_type or_nolink = null_affinity) {
  affinity_type or_sum = 0;
  size_type or_count = 0;
  for (; first1 != last1 || first2 != last2; ++or_count) {
    for (; first1 != last1 && !care1(iter_termid(first1)); ++first1) {
    }
    for (; first2 != last2 && !care2(iter_termid(first2)); ++first2) {
    }
...
affinity_type affinity_measure(const CrossList<force_type>& waf_mat1,
                               termid_type i1, Predicate1 back1,
                               const CrossList<force_type>& waf_mat2,
                               termid_type i2, Predicate2 back2,
  ...
  affinity_type k_mean = affinity_or_mean(
      ...
      std::mem_fn(&CrossList<force_type>::const_column_iterator::row),
      NO_LINK_FLAG);
  ...
  affinity_type l_mean = affinity_or_mean(
      ...
      std::mem_fn(&CrossList<force_type>::const_row_iterator::column),
      NO_LINK_FLAG);
  ...
  if (!has_inlink && !has_outlink) {
    return affinity_nolink;
  }
...
void word_activation_force(const CrossList<cooccur_type>& co_mat,
                           Predicate1 care_left, Predicate2 care_right,
                           UnaryFunction term_freq, force_type prec,
                           CrossList<force_type>& waf_mat) {
  waf_mat.clear();
  size_type term_size = std::max(co_mat.row_count(), co_mat.column_count());
  waf_mat.reserve(term_size, term_size);
  for (auto co_iter = co_mat.begin(), co_end = co_mat.end(); co_iter != co_end;
       ++co_iter) {
    // verify there is at least one term(id) concerned
    termid_type i = co_iter.row(), j = co_iter.column();
    if (!care_left(i) || !care_right(j)) {
      continue;
    }

    // calculate waf from term i to term j which the element denotes
    force_type waf =
        word_activation_force(*co_iter, term_freq(i), term_freq(j));
...
template <typename Predicate1, typename Predicate2, typename UnaryFunction>
void word_activation_force(std::istream& co_mat_is, Predicate1 care_left,
                           Predicate2 care_right, UnaryFunction term_freq,
                           force_type prec, std::ostream& waf_mat_os) {
  ...
  for (char beg_ch = 0; co_mat_is >> beg_ch;) {
    ...
    switch (beg_ch) {
      case '(':  // cell
        ...
        if (care_left(co_cell.row) && care_right(co_cell.column)) {
          // calculate waf value
          force_type waf = word_activation_force(
              co_cell.value, term_freq(co_cell.row), term_freq(co_cell.column));
          if (waf < prec) {
            break;
          }
    ...
    if (unknown_pattern) {
      break;  // unknown pattern, break loop
    }
  }
}

template <typename Predicate1, typename Predicate2>
void affinity_measure(const CrossList<force_type>& waf_mat, Predicate1 care,
                      Predicate2 back, affinity_type prec,
                      affinity_type affinity_nolink,
                      CrossList<affinity_type>& a_mat) {
  // argument checking
  if (waf_mat.row_count() != waf_mat.column_count()) {
    std::stringstream ss;
    ss << "waf::affinity_measure(waf_mat, care, back, prec, a_nolink, "
          "a_mat):\n";
    ss << "waf_mat row size must be exactly the same as column size, but\n";
    ss << "\twaf_mat: row=" << waf_mat.row_count()
       << ", column=" << waf_mat.column_count() << "\n";
  ...
  for (size_type i = 0; i < term_size; ++i) {
    ...
    for (size_type j = i; j < term_size; ++j) {
      ...
      affinity_type a =
          affinity_measure(waf_mat, i, back, waf_mat, j, back, affinity_nolink);
      if (0 <= a && a < prec) {
        continue;
      }
      a_mat.rset(i, j, a);
...
void affinity_measure(const CrossList<force_type>& waf_mat, Predicate1 care,
  ...
  for (size_type i = 0; i < term_size; ++i) {
    ...
    auto row_iter = a_mat.row_begin(i), row_end = a_mat.row_end(i);
    for (; row_iter != row_end; ++row_iter) {
      a_mat_os << a_cell_type(i, row_iter.column(), *row_iter);
    }

    // erase this row from a_mat
    if (row_iter = a_mat.row_begin(i); row_iter != row_end) {
...
void affinity_measure(const CrossList<force_type>& waf_mat1, Predicate1 back1,
  ...
  for (termid_type i = 0; i < term_size; ++i, ++term_a_iter) {
    *term_a_iter = affinity_measure(waf_mat1, i, back1, waf_mat2, i, back2,
                                    affinity_nolink);
  }
}
...
void affinity_measure(const CrossList<force_type>& waf_mat1, Predicate1 back1,
  ...
  for (termid_type i = 0; i < term_size; ++i) {
    for (termid_type j = 0; j < term_size; ++j) {
      a_mat.rat(i, j) = affinity_measure(waf_mat1, i, back1, waf_mat2, j, back2,
                                         affinity_nolink);
    }
  }
}
...
void affinity_measure(const CrossList<force_type>& waf_mat1, Predicate1 back1,
  ...
  for (termid_type i = 0; i < term_size; ++i) {
    for (termid_type j = 0; j < term_size; ++j) {
      affinity_type a_value = affinity_measure(waf_mat1, i, back1, waf_mat2, j,
                                               back2, affinity_nolink);
...
template <typename Predicate1, typename Predicate2, typename Predicate3,
          typename Predicate4>
void affinity_measure(const CrossList<force_type>& waf_mat1, Predicate1 care1,
                      Predicate2 back1, const CrossList<force_type>& waf_mat2,
                      Predicate3 care2, Predicate4 back2, affinity_type prec,
                      affinity_type affinity_nolink,
                      CrossList<affinity_type>& a_mat) {
  // argument checking
  if (waf_mat1.row_count() != waf_mat1.column_count() ||
