...
template <typename AccumulateT, typename PreciseT = long double,
          typename CountT = std::size_t>
class StatVar {
 public:
 ...
 public:
  StatVar(const std::string& var_name = "") : name(var_name) {}
  ...
  StatVar& operator+=(const StatVar& other) {
    ...
    if (hist_enabled_) {
      ...
      std::transform(bins_.begin(), bins_.end(), other.bins_.begin(),
                     bins_.begin(), std::plus<count_type>());
    }
    return *this;
  }
  ...
  template <typename T>
  T mean() const {
    return static_cast<T>(sum_) / static_cast<T>(counter_);
  }

  template <typename T>
  T variance() const {
    T square_mean = static_cast<T>(square_sum_) / static_cast<T>(counter_);
    T record_mean = mean();
  ...
  friend std::ostream& operator<<(std::ostream& out, const StatVar& var) {
    out << var.name << " sum=" << var.sum() << " count=" << var.count()
        << " mean=" << var.mean() << " min=" << var.min()
        << " max=" << var.max() << " variance=" << var.variance()
        << " deviation=" << var.deviation();
    if (var.hist_enabled_) {
      out << "bin\tcount" << std::endl;
      out << "[less, " << var.thresholds_[0] << ")\t" << var.bins_[0]
          << std::endl;
      std::size_t i = 0;
      for (i = 1; i < var.thresholds_.size(); ++i) {
        out << "[" << var.thresholds_[i - 1] << ", " << var.thresholds_[i]
            << ")\t" << var.bins_[i] << std::endl;
      }
      out << "[" << var.thresholds_[i - 1] << ", more)\t" << var.bins_[i]
          << std::endl;
    }
    return out;
  }
  ...
  void set_hist_thresholds(InputIterator first, InputIterator last) {
    ...
    bins_.assign(thresholds_.size() + 1, 0.0);

    // no thresholds input to disable histogram
    hist_enabled_ = !thresholds_.empty();
  }

  void set_hist_linear_thresholds(accumulate_type start, accumulate_type step,
                                  accumulate_type stop) {
  ...
  void set_hist_exponential_thresholds(accumulate_type radix,
                                       accumulate_type pow_start,
                                       accumulate_type pow_step,
                                       accumulate_type pow_stop,
                                       accumulate_type times = 1) {
    std::vector<accumulate_type> values;
    for (accumulate_type power = pow_start; power <= pow_stop;
         power += pow_step) {
  ...
  const std::vector<accumulate_type>& hist_thresholds() const {
    return thresholds_;
  }
