...
inline std::string to_string(Level level, size_t width = 0) {
  ...
  if (width > name.size()) name.append(width - name.size(), ' ');
  return name;
}
...
class Logger {
 ...
 public:  // key functions
  ...
  void write_indents() {
    for (IndentStack::const_iterator ii = indents_.begin();
         ii != indents_.end(); ++ii) {
      (*this) << (*ii);
    }
  }
...
class Indent {
 public:
  Indent(const std::string& info = "    ", Logger& logger = standard_logger())
      : logger_(logger) {
    logger_.indent(info);
  }
...
class Attach {
 public:
  Attach(Level level, std::ostream& sink, Logger& logger = standard_logger())
...
class NamedLogger {
 public:
  ...
  template <typename T>
  Logger& operator<<(const T& value) {
    logger_->dispatch_message(value);
    return *this;
  }
  Logger& operator<<(std::ostream& (*pf)(std::ostream&)) {
    return operator<<<std::ostream& (*)(std::ostream&)>(pf);
...
inline NamedLogger named_logger(const std::string& name,
                                Logger& logger = standard_logger()) {
