...
template <typename T>
SparseMatrix<T>::SparseMatrix(size_type row_count, size_type column_count,
...
SparseMatrix<T>& SparseMatrix<T>::operator=(const SparseMatrix& rhs) {
  ...
  sparse(sp.first, sp.second);  // PA: sparse(row,col) must be called before
...
template <typename T>
void SparseMatrix<T>::sparse(size_type sparse_row_count,
                             size_type sparse_column_count) {
...
std::pair<typename SparseMatrix<T>::size_type,
          typename SparseMatrix<T>::size_type>
SparseMatrix<T>::sparse() const {
  assert(!table_.empty());
...
bool SparseMatrix<T>::ierase(size_type row_index, size_type col_index) {
  if (!is_valid_row(row_index) || !is_valid_column(col_index))
    throw std::out_of_range(
        "SparseMatrix<T>::rerase(size_type,size_type): row_index or col_index "
        "illegal");
...
template <typename T>
typename SparseMatrix<T>::reference SparseMatrix<T>::iat(size_type row_index,
                                                         size_type col_index) {
  ...
  if (p) {                                  // if there is a node there
    ...
    return this->at(row_index, col_index);  // return at(row_index,col_index)
  }
}

template <typename T>
typename SparseMatrix<T>::const_reference SparseMatrix<T>::iat(
...
template <typename T>
typename SparseMatrix<T>::value_type SparseMatrix<T>::iget(
    size_type row_index, size_type col_index) const {
  if (!is_valid_row(row_index) || !is_valid_column(col_index)) {
    throw std::out_of_range(
...
template <typename T>
void SparseMatrix<T>::iset(size_type row_index, size_type col_index,
                           const_reference value) {
  if (!is_valid_row(row_index) || !is_valid_column(col_index)) {
    throw std::out_of_range(
        "SparseMatrix<T>::iset(size_type,size_type,const_reference): row_index "
...
template <typename T>
typename SparseMatrix<T>::node* SparseMatrix<T>::ilocate(
    size_type row_index, size_type col_index) const {
  ...
  for (typename std::list<node*>::iterator iter = nodes.begin();
       iter != nodes.end(); ++iter) {
    if (row_index == (*iter)->row && col_index == (*iter)->column) return *iter;
  }
  return NULL;
}

template <typename T>
void SparseMatrix<T>::insert_node_before(node* new_node,
...
template <typename T>
void SparseMatrix<T>::insert_node_after(node* new_node,
                                        const std::pair<node*, node*>& ptrs) {
