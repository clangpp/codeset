...
void shell_sort(RandomAccessIterator first, RandomAccessIterator last,
  ...
  typedef typename std::iterator_traits<RandomAccessIterator>::value_type
      value_type;
  typedef
      typename std::iterator_traits<BidirectionalIterator>::value_type gap_type;
...
OutputIterator merge(InputIterator1 first1, InputIterator1 last1,
  ...
  return sorting::merge(first1, last1, first2, last2, result,
                        std::less<value_type>());
}
...
BidirectionalIterator partition(BidirectionalIterator first,
  ...
  while (first != last) {
    ...
    if (first != last) {
      --last;  // right sentry move to test element
    }
...
void quick_sort(RandomAccessIterator first, RandomAccessIterator last,
  ...
  center = sorting::partition(
      first + 1, last - 1,
      [&](const decltype(*center)& value) { return pred(value, *center); });
...
class IndirectCompare : public std::binary_function<Iterator, Iterator, bool> {
 public:
...
void indirect_sort(RandomAccessIterator first, RandomAccessIterator last,
  ...
  quick_sort(
      ...
      internal::IndirectCompare<BinaryPredicate, RandomAccessIterator>(pred));
  ...
  for (difference_type i = 0, len = iters.size(); i < len; ++i) {
    if (iters[i] == first + i) {
      continue;  // matched, no need to move
    }
...
template <typename ForwardIterator>
ForwardIterator bucket_sort(ForwardIterator first, ForwardIterator last,
                            bool ascending = true) {
...
RandomAccessIterator quick_select(RandomAccessIterator first,
                                  RandomAccessIterator last, Distance n,
                                  BinaryPredicate pred) {
  if (n < 0 || last - first <= n) {
    return last;  // illegal input
  }
  if (last - first == 1 && n == 0) {
    return first;  // return condition
  }
  ...
  RandomAccessIterator center = internal::median3(first, last, pred);
  center = sorting::partition(first, last, [&](const decltype(*center)& value) {
    return pred(value, *center);
  });
  if (n < center - first) {
